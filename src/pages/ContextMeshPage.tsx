...
import { ArrowLeft, Waypoints, Puzzle, Sparkles, Radio, Layers, ExternalLink, ArrowRight, BookOpen, ChevronDown } from 'lucide-react'
...
import { NewMcpFromApisWizard } from '../components/NewMcpFromApisWizard'
...
function ContextMeshLanding() {
  ...
  const [wizard, setWizard] = useState<{ registryId: string } | null>(null)
  const [fromApisOpen, setFromApisOpen] = useState(false)
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {mode === 'creation' && (
          <div className="relative" ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-1.5 w-52 bg-[#0f1318] border border-kong-border rounded-lg shadow-xl z-20 py-1 overflow-hidden">
                <button
                  onClick={() => { setDropdownOpen(false); setWizard({ registryId: 'meridian-internal-registry' }) }}
                  className="w-full text-left px-4 py-2.5 text-[13px] text-kong-text hover:bg-white/[0.05] transition-colors"
                >
                  <div className="font-medium">Register MCP Server</div>
                  <div className="text-[11px] text-kong-text-muted mt-0.5">Point to an existing MCP server</div>
                </button>
                <div className="h-px bg-kong-border mx-2" />
                <button
                  onClick={() => { setDropdownOpen(false); setFromApisOpen(true) }}
                  className="w-full text-left px-4 py-2.5 text-[13px] text-kong-text hover:bg-white/[0.05] transition-colors"
                >
                  <div className="font-medium">Build from REST APIs</div>
                  <div className="text-[11px] text-kong-text-muted mt-0.5">Compose tools from catalog APIs</div>
                </button>
              </div>
            )}
          </div>
      ...
      {wizard && (
        <NewMcpWizard
          ...
          initialRegistryId={wizard.registryId}
          onClose={() => setWizard(null)}
        />
      )}
      {fromApisOpen && (
        <NewMcpFromApisWizard onClose={() => setFromApisOpen(false)} />
      )}
    </div>
  )
}
